...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/rules/actions/index.ts"
      provides: "Import and registration of createLockdownPlaceMilitiaAction"
      contains: "createLockdownPlaceMilitiaAction"
  ...
  key_links:
    ...
    - from: "src/rules/actions/index.ts"
      to: "src/rules/actions/dictator-actions.ts"
      via: "imports and registers createLockdownPlaceMilitiaAction"
      pattern: "createLockdownPlaceMilitiaAction"
---
...
@src/rules/actions/index.ts (action registration — add new import + registerAction call)
...
<task type="auto">
  <name>Task 2: Create lockdownPlaceMilitia action, register it, and add flow step</name>
  <files>
    src/rules/actions/dictator-actions.ts
    src/rules/actions/index.ts
    src/rules/flow.ts
...
Action.create('lockdownPlaceMilitia')
  ...
  .chooseFrom<string>('amount', {
    ...
    choices: () => {
      ...
      // IMPORTANT: Cap must account for current militia in the selected sector.
      // Since we don't know which sector was selected yet at this point, use
      // Math.min(remaining, 10) as upper bound. The execute handler will enforce
      // the actual sector cap (10 - sector.dictatorMilitia) and refund overflow.
      // However, to avoid misleading UX, the targetSector choices already filter
      // out sectors at cap (dictatorMilitia >= 10), so the player can only pick
      // sectors with room. The ideal max here is Math.min(remaining, 10 - selectedSectorMilitia),
      // but since chooseFrom evaluates before sector selection is known, we approximate.
      // A practical approach: offer 1 through Math.min(remaining, 10) and validate in execute.
...
1. Add `createLockdownPlaceMilitiaAction` to the import from `'./dictator-actions.js'` (alongside `createGeneralissimoPickAction` added by plan 03).

2. Add `game.registerAction(createLockdownPlaceMilitiaAction(game));` in the `registerAllActions` function, near the other dictator per-turn ability actions (after `createGeneralissimoPickAction`).

Without this registration, the flow step will fail with "Action not available" when it tries to list `lockdownPlaceMilitia`.
...
After the lockdown placement loop, add combat handling loops (same set of loops that already exist for tactics combat: before-attack-healing, attack-dog, target-selection, hit-allocation, wolverine-sixes, epinephrine, continue-or-retreat, animation-wait). Actually — check if the EXISTING tactics-combat loops (already present at lines 583-701) will handle combat triggered by lockdown. If they're already in the flow after the lockdown loop position, they will naturally handle any combat queued by lockdown placement. Verify the loop order and confirm no additional combat loops are needed.
  ...
  <verify>
    `npx vitest run` passes. Grep for `lockdownPlaceMilitia` in flow.ts, dictator-actions.ts, and index.ts — all three should have hits. Grep for `createLockdownPlaceMilitiaAction` in index.ts — should appear in both import and registerAction call. Grep for `pendingLockdownMilitia` in game.ts, tactics-effects.ts, dictator-actions.ts, and flow.ts.
  </verify>
  <done>
    Human dictator gets iterative militia placement on base/adjacent sectors. Flow yields for each placement decision. Combat triggers when rebels are present. All militia placed or sector caps reached. Action is properly registered in index.ts so flow steps can find it.
  </done>
</task>
...
4. `grep -rn "lockdownPlaceMilitia" src/rules/` returns hits in dictator-actions.ts, flow.ts, and index.ts
5. `grep -rn "createLockdownPlaceMilitiaAction" src/rules/actions/index.ts` returns hits in both import block and registerAllActions function
...
- 10-per-sector cap is respected (both in choices filtering and execute validation)
- Combat triggered when militia placed in rebel-occupied sectors
- Action is registered in index.ts (no "Action not available" errors)
