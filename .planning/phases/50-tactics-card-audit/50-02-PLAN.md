...
must_haves:
  truths:
    - "Every correctly-implemented tactics card fires a game.animate() call when executed"
    - "GameTable.vue has registered animation handlers for all tactics animation events"
    - "Animations use pre-computed data and put mutations inside the callback"
  artifacts:
    - path: "src/rules/tactics-effects.ts"
      provides: "game.animate() calls in 11 working effect functions"
...
<objective>
Add visible animations to all 11 correctly-implemented tactics card effects and register their animation handlers in GameTable.vue.
...
Output: All 11 working tactics effects call `game.animate()` with pre-computed data and mutations in callback. GameTable.vue registers handlers for each event type.
</objective>
...
<task type="auto">
  <name>Task 1: Add game.animate() calls to all 11 working tactics effects</name>
  <files>
    src/rules/tactics-effects.ts
  </files>
  <action>
For each of the 11 correctly-implemented effect functions, wrap the state mutations inside a `game.animate()` call. Follow the pattern documented in MEMORY.md and used by mortar-strike/combat-panel:
...
**Cards grouped by animation type:**

**Sector-targeted animations (show affected sectors on map):**
1. `artilleryBarrage` — Already has sector targeting logic. Add `game.animate('tactic-artillery-barrage', { sectorsTargeted: sectorRolls.map(s => ({ sectorId: s.sector.sectorId, sectorName: s.sector.sectorName, hits: s.hits })) }, () => {})` BEFORE the pending state is set. This is a pure UI signal since mutations happen through the allocation flow.

2. `familyThreat` — Pre-compute `affectedSectors` array with sectorId/sectorName/removed counts. Wrap the militia removal loop inside `game.animate('tactic-family-threat', { affectedSectors }, () => { /* removal mutations here */ })`.

3. `fodder` — Pre-compute target sectors and militia counts. Wrap the militia placement + combat queueing inside `game.animate('tactic-fodder', { targets: [...] }, () => { /* mutations */ })`.

4. `reinforcements` — Pre-compute which industries get reinforced. Wrap militia additions inside `game.animate('tactic-reinforcements', { sectors: [...] }, () => { /* mutations */ })`.

5. `seizure` — Pre-compute which wilderness sectors will flip. Wrap explore + militia placement inside `game.animate('tactic-seizure', { sectors: [...] }, () => { /* mutations */ })`.

6. `sentry` — Pre-compute which uncontrolled sectors get militia. Wrap placement inside `game.animate('tactic-sentry', { sectors: [...] }, () => { /* mutations */ })`.

7. `blockTrade` — Pre-compute which cities flip. Wrap city explore + militia inside `game.animate('tactic-block-trade', { cities: [...] }, () => { /* mutations */ })`. Note: This function may have been updated by plan 01. Read it before modifying.

8. `taintedWater` — Pre-compute militia removal counts and MERC damage targets. Wrap mutations inside `game.animate('tactic-tainted-water', { sectors: [...], mercs: [...] }, () => { /* mutations */ })`.

**Banner/flag animations (permanent effects, no board-visible change):**
9. `betterWeapons` — The state mutation is just setting a boolean flag. Use `game.animate('tactic-better-weapons', { effectName: 'Better Weapons', description: 'Militia now hit on 3+' }, () => { game.betterWeaponsActive = true; })`. The `revealBase()` call should happen BEFORE animate.

10. `veteranMilitia` — Same pattern. `game.animate('tactic-veteran-militia', { effectName: 'Veteran Militia', description: 'Militia +1 initiative' }, () => { game.veteranMilitiaActive = true; })`. The `revealBase()` call should happen BEFORE animate.

11. `conscripts` — `game.animate('tactic-conscripts', { effectName: 'Conscripts', amount: Math.ceil(game.rebelCount / 2) }, () => { game.conscriptsActive = true; game.conscriptsAmount = Math.ceil(game.rebelCount / 2); })`.

12. `oilReserves` — `game.animate('tactic-oil-reserves', { effectName: 'Oil Reserves', description: 'Oil controller gets free action' }, () => { game.oilReservesActive = true; })`.
...
For each animation event, include the card name in the data so the UI can display it: `{ cardName: 'Family Threat', ...specificData }`.
  ...
  <verify>
    Run `npx vitest run` — all tests pass. Grep for `game.animate` in tactics-effects.ts — should find 11-12 occurrences (one per working card effect function, excluding generalisimo and lockdown).
  </verify>
  <done>
    All 11 working tactics card effects call game.animate() with appropriate data and mutations in callbacks. Theatre view correctly shows pre-animation state for each card.
  </done>
</task>

<task type="auto">
  <name>Task 2: Register animation handlers in GameTable.vue</name>
  <files>
    src/ui/components/GameTable.vue
  </files>
  <action>
...
Register a handler for each event type. Each handler sets `activeTacticEvent.value` with the event data and waits for resolution:
```typescript
animationEvents.registerHandler('tactic-family-threat', async (event) => {
  ...
  await new Promise<void>((resolve) => { tacticEventResolve = resolve; });
});
```

For the auto-dismiss pattern (since we don't have dedicated tactic animation components yet), use a timeout to auto-resolve after 2 seconds:
```typescript
animationEvents.registerHandler('tactic-family-threat', async (event) => {
  activeTacticEvent.value = { type: 'family-threat', cardName: 'Family Threat', data: event.data as Record<string, unknown> };
...
2. `grep -c "game.animate" src/rules/tactics-effects.ts` returns 11-12
...
<success_criteria>
- All 11 working tactics cards fire visible animations
