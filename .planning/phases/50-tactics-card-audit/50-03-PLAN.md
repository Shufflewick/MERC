...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/rules/actions/index.ts"
      provides: "Import and registration of createGeneralissimoPickAction"
      contains: "createGeneralissimoPickAction"
  ...
  key_links:
    ...
    - from: "src/rules/actions/index.ts"
      to: "src/rules/actions/dictator-actions.ts"
      via: "imports and registers createGeneralissimoPickAction"
      pattern: "createGeneralissimoPickAction"
---
...
<task type="auto">
  <name>Task 2: Create generalissimoPick action, register it, and add flow step</name>
  <files>
    src/rules/actions/dictator-actions.ts
    src/rules/actions/index.ts
    src/rules/flow.ts
...
1. Add `createGeneralissimoPickAction` to the import from `'./dictator-actions.js'` (line 96-100 area, alongside `createCastroBonusHireAction` and `createKimBonusMilitiaAction`).

2. Add `game.registerAction(createGeneralissimoPickAction(game));` in the `registerAllActions` function, near the other dictator per-turn ability actions (lines 201-203 area, after `createKimBonusMilitiaAction`).

Without this registration, the flow step will fail with "Action not available" when it tries to list `generalissimoPick`.
...
Place this AFTER the artillery-allocation loop at line 579 and BEFORE the tactics-combat-before-attack-healing loop at line 583.
  ...
  <verify>
    `npx vitest run` passes. Grep for `generalissimoPick` in flow.ts, dictator-actions.ts, and index.ts — all three should have hits. Grep for `createGeneralissimoPickAction` in index.ts — should appear in both import and registerAction call.
  </verify>
  <done>
    Human dictator gets interactive MERC selection after playing Generalissimo. The flow step yields for player input, the action handles draw-6-pick-1 with squad placement and equipment, and the action is properly registered in index.ts so flow steps can find it.
  </done>
</task>
...
4. `grep -rn "generalissimoPick" src/rules/` returns hits in dictator-actions.ts, flow.ts, and index.ts
5. `grep -rn "createGeneralissimoPickAction" src/rules/actions/index.ts` returns hits in both import block and registerAllActions function
</verification>
