...
<task type="auto">
  <name>Task 2: Emit combat-panel snapshot at all 8 decision cycle points in executeCombat</name>
  <files>src/rules/combat.ts</files>
  <action>
Add `game.animate('combat-panel', buildCombatPanelSnapshot(game))` after every `game.activeCombat = { ... }` assignment in `executeCombat()` and after every point where the combat pauses for a player decision. There are 8 emission points:
...
**Point 5: Paused for epinephrine decision (~between line 2600 and 2602)**
This is the CRITICAL addition. When `executeCombatRound` returns with `pausedForEpinephrine: true`, `pendingEpinephrine` is already set on `game.activeCombat` inside `executeCombatRound`. However, `executeCombat` has no explicit handler for this pause -- it falls through to the "Round completed" section. The flow engine then presents the epinephrine action step to the player.

Add a new block after the `pausedForBeforeAttackHealing` handler and before the `pausedForAttackDogSelection` handler:
```typescript
...
IMPORTANT: This block must follow the same pattern as the other pause handlers. It spreads `...game.activeCombat!` first to preserve `pendingEpinephrine` (which was already set by `executeCombatRound`), then overlays the standard combat state fields. The `currentAttackerIndex` from `roundResult` preserves the resume point. Use the same return shape as the other pause handlers (combatPending: true, canRetreat: false).

NOTE: If `roundResult` does not have a `currentAttackerIndex` property when paused for epinephrine, check the return type from `executeCombatRound` and use the appropriate field to capture the attacker index. The epinephrine pause happens mid-round during casualty processing, so the attacker index may be on `roundResult` or may need to be derived from the round results length.

**Point 6: Paused for attack dog selection (~line 2607)**
...
**Point 7: Paused for retreat/continue (~line 2695)**
...
**Point 8: Combat complete (~line 2770)**
...
Count combat-panel emissions: `grep -c "game.animate('combat-panel'" src/rules/combat.ts` should return 8.
...
Verify the epinephrine block exists: `grep -A2 'pausedForEpinephrine' src/rules/combat.ts` should show the new handler block in executeCombat.
  </verify>
  <done>8 combat-panel snapshot emissions in executeCombat covering: combat start, target selection, hit allocation, before-attack healing, epinephrine decision, attack dog selection, retreat/continue, and combat complete. All tests pass. Decision context is embedded in each snapshot via buildCombatPanelSnapshot reading activeCombat.</done>
</task>
...
2. `grep -c "game.animate('combat-panel'" src/rules/combat.ts` -- returns 8
3. `grep -n "buildCombatPanelSnapshot" src/rules/combat.ts` -- shows function definition + 8 call sites
...
6. Verify epinephrine handler: `grep -B2 -A5 'pausedForEpinephrine' src/rules/combat.ts` shows handler block in executeCombat with combat-panel emission
</verification>

<success_criteria>
- combat-panel snapshot emitted at every decision cycle point (8 points)
...
- Epinephrine decision cycle has dedicated handler with combat-panel snapshot (SRV-02)
