...
must_haves:
  truths:
    ...
    - "All combat-heal events carry healthBefore, healthAfter, and healAmount fields"
  artifacts:
    ...
    - path: "src/rules/actions/rebel-combat.ts"
      provides: "3 pure-data animate calls (callbacks removed) with healthBefore field"
...
<task type="auto">
  <name>Task 2: Extract mutations from all 3 animate callbacks in rebel-combat.ts and add healthBefore field</name>
...
**IMPORTANT (SRV-03 compliance): Add `healthBefore: targetCombatant.health` to the event data object in ALL 3 combat-heal animate calls.** The value is the target's health BEFORE healing is applied. Since mutations are being moved OUT of the callback to after the animate call, `targetCombatant.health` at the point of the animate call is naturally the pre-heal value. This field is required by SRV-03 alongside the existing `healAmount` and `healthAfter` fields.

**Call 11: combat-heal (medical kit heal action) ~line 811**
Add `healthBefore` to event data and move the entire callback body after animate:
```
...
Same pattern as Call 11. Add `healthBefore: targetCombatant.health` to the event data and move entire callback body after animate. Note: the `beforeAttackHealingProcessed` set update and `pendingBeforeAttackHealing = undefined` already happen AFTER the animate call (lines 1012-1016), so they stay where they are.

**Call 13: combat-heal (surgeon ability) ~line 1245**
Add `healthBefore: targetCombatant.health` to the event data and move callback body after animate:
```
game.animate('combat-heal', {
...
For all three: keep `healingItem` and other local variable references intact. The variables are in closure scope and remain accessible since the mutations now run in the same scope as the animate call.
  ...
  <verify>
Run `npx vitest run` -- all existing tests must pass. Grep for callback patterns: `game.animate\(.*\) => \{` should return zero results in rebel-combat.ts. Verify healthBefore field: `grep -n 'healthBefore' src/rules/actions/rebel-combat.ts` should show 3 occurrences (one per combat-heal call).
  </verify>
  <done>All 3 animate calls in rebel-combat.ts are pure data (no callback argument). All 3 combat-heal events carry healthBefore, healthAfter, and healAmount. All combat tests pass unchanged.</done>
</task>
...
5. `grep -n 'healthBefore' src/rules/actions/rebel-combat.ts` -- returns 3 occurrences
</verification>

<success_criteria>
- All 13 game.animate() calls across combat.ts and rebel-combat.ts are pure data with no callbacks
- All 3 combat-heal events carry healthBefore, healthAfter, and healAmount fields (SRV-03)
