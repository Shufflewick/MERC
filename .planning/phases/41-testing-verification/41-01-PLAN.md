...
Each test should verify BOTH effectiveStat value AND activeStatModifiers content.
  ...
  <verify>npm test -- --reporter=verbose merc-abilities-integration | grep -E "(Equipment-Conditional|PASS|FAIL)"</verify>
...
- Test that effectiveCombat === 3 (not just that it exists)
  ...
  <verify>npm test -- --reporter=verbose merc-abilities-integration | grep -E "(Passive|Shooter|Juicer|Ewok|PASS|FAIL)"</verify>
