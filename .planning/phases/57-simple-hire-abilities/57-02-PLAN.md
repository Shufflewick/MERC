...
must_haves:
  truths:
    ...
    - "useActionState return type interface includes isGadafiHiring and isStalinHiring"
    - "No dead-code props created (isCastroHiring prop in HiringPhase.vue is unused -- do not replicate)"
  artifacts:
...
@src/ui/composables/useActionState.ts (lines 40-55 for return type interface, lines 170-175, 226-232, 243-249, 320-325)
@src/ui/components/GameTable.vue (line 190 for destructure, line 903 for hiringActions)
</context>
...
5. **useActionState.ts** -- there are 4 locations referencing `castroBonusHire` PLUS the return type interface. Update ALL of these:

   - **Lines 40-55** (return type interface): The TypeScript interface that types the composable's return value lists `isCastroHiring: ComputedRef<boolean>`. Add `isGadafiHiring: ComputedRef<boolean>` and `isStalinHiring: ComputedRef<boolean>` to this interface. Without this, TypeScript will error when returning the new computeds.
...
7. **GameTable.vue** (~line 190): Add `isGadafiHiring` and `isStalinHiring` to the destructure from `useActionState()`. Follow the pattern of how `isCastroHiring` is destructured.

8. **HiringPhase.vue -- DO NOT add props.** The `isCastroHiring` prop threaded from GameTable to HiringPhase.vue is declared but NEVER USED in its template -- it is dead code. Do NOT create `isGadafiHiring`/`isStalinHiring` equivalents for HiringPhase. Do not propagate dead code.
  </action>
  <verify>
Run `npx tsc --noEmit` to confirm no type errors. Then grep all source files for `castroBonusHire` and verify that every array/conditional containing it also contains `gadafiBonusHire` and `stalinBonusHire` (except HiringPhase.vue prop which is dead code).
  </verify>
  <done>
Flow step lists all 4 dictator ability actions. All 3 UI files route the new action names correctly. useActionState return type interface updated. GameTable destructure updated. No dead-code props added to HiringPhase. No `castroBonusHire` reference exists without corresponding Gaddafi/Stalin entries (except dead HiringPhase prop).
  </done>
</task>
...
3. Grep the codebase for any remaining references to `castroBonusHire` that might indicate a missed UI wiring point. Every array containing `castroBonusHire` should also contain `gadafiBonusHire` and `stalinBonusHire` (exception: HiringPhase.vue `isCastroHiring` prop is dead code, ignore it).
...
3. Every occurrence of `castroBonusHire` in UI files has corresponding `gadafiBonusHire` and `stalinBonusHire` entries (except dead HiringPhase prop)
4. Flow step `dictator-ability` lists all 4 ability action names
5. useActionState return type interface includes isGadafiHiring and isStalinHiring
</verification>

<success_criteria>
- Flow step includes gadafiBonusHire and stalinBonusHire
- All 4 useActionState.ts locations updated (lines 172, 229, 246, 323) plus return type interface (lines 40-55)
- DictatorPanel.vue routes both new actions
- GameTable.vue hiringActions includes both new actions and destructures new computeds
- No dead-code props added to HiringPhase.vue
