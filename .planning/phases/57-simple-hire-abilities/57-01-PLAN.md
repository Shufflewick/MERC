...
1. Add `applyGadafiTurnAbility(game: MERCGame): DictatorAbilityResult` function:
   ...
   - Import `buildMapCombatantEntry` and `emitMapCombatantEntries` from `'./animation-events.js'` (same directory -- both files are in `src/rules/`)
...
3. Update `applyDictatorTurnAbilities` switch statement (~line 301):
  ...
  <verify>
Run `npx tsc --noEmit` to confirm no type errors in dictator-abilities.ts.
  </verify>
  <done>
Both AI-path functions exist with switch cases. Import uses correct `'./animation-events.js'` path. TypeScript compiles clean.
  </done>
</task>
...
4. Import path in dictator-abilities.ts uses `'./animation-events.js'` (same directory)
</verification>
