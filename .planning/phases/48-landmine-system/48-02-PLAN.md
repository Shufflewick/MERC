...
<task type="auto">
  ...
  <action>
    ...
    **Execute Coordinated Attack action (lines 603-631):** Collect squads DURING the existing movement for-loop (lines 603-617) instead of re-iterating participants after `clearCoordinatedAttack()`. The loop already resolves each participant to a squad object -- collect them into an array as you go:
    ...
    Then insert the landmine check after `game.clearCoordinatedAttack(targetId)` (line 620) but BEFORE `hasEnemies()` check (line 626):
    ...
    The `true` is hardcoded because executeCoordinatedAttack is rebel-only.
