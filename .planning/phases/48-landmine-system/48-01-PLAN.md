...
must_haves:
  truths:
    - "A detonated landmine deals 1 damage to every enemy combatant in the sector"
...
<feature>
  <name>checkLandMines -- bidirectional landmine trigger</name>
  ...
  <behavior>
    ...
    2. Mine in stash, no Squidhead in entering squad, entering player is rebel ->
       deals 1 damage to EVERY rebel merc in sector, kills 1 rebel militia per rebel player in sector,
    ...
    3. Mine in stash, no Squidhead, entering player is dictator ->
       deals 1 damage to EVERY dictator merc in sector, kills 1 dictator militia in sector,
       removes mine from stash, discards mine -> { detonated: true, disarmed: false }
  ...
  <implementation>
    ...
    7. Inside `game.animate('landmine-detonate', {...}, () => { ... })` callback:
       ...
       - Kill 1 militia per faction (rebel: `sector.removeRebelMilitia(playerId, 1)` per rebel player; dictator: `sector.removeDictatorMilitia(1)`)
...
- Tests cover: no mine (no-op), detonation damages entering player's units, mine discarded after detonation, Squidhead prevents detonation, Squidhead disarm sends mine to discard
