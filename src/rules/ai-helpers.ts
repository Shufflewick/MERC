...
import { Sector, MercCard, Equipment, CombatUnit } from './elements.js';
import {
  isLandMine,
  isRepairKit,
  isEpinephrine,
...
export function autoEquipDictatorUnits(game: MERCGame, sector: Sector): number {
  // Get all dictator units in this sector (MercCards + DictatorCard share CombatUnit base)
  const units: CombatUnit[] = game.dictatorPlayer.hiredMercs.filter(m => m.sectorId === sector.sectorId);
