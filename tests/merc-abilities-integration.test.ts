...
describe('MERC Ability Integration Tests', () => {
  ...
  describe('Team Restrictions', () => {
    ...
    it('Cannot hire incompatible MERC with multiple team members', () => {
      const team = [{ combatantId: 'basic' }, { combatantId: 'squirrel' }] as MercCard[];
      expect(canHireMercWithTeam('borris', team)).toBe(false);
      expect(canHireMercWithTeam('lucid', team)).toBe(true);
    });
  });
  ...
  describe('Equipment Restrictions', () => {
    ...
    describe('Apeiron - Won\'t use grenades or mortars', () => {
      ...
      it('Apeiron canEquip should reject grenades', () => {
        ...
        const apeiron = game.mercDeck.all(MercCard).find(m => m.combatantId === 'apeiron');
        if (!apeiron) {
    ...
    describe('Genesis - Weapon in accessory slot', () => {
      it('Genesis should allow weapons in accessory slot', () => {
        const genesis = game.mercDeck.all(MercCard).find(m => m.combatantId === 'genesis');
        if (!genesis) {
    ...
    describe('Gunther - All slots can be accessories', () => {
      it('Gunther should accept accessories in weapon slot', () => {
        const gunther = game.mercDeck.all(MercCard).find(m => m.combatantId === 'gunther');
        if (!gunther) {
  ...
  describe('Health and Action Modifiers', () => {
    ...
    describe('Juicer - +2 health', () => {
      it('Juicer should have 5 max health instead of 3', () => {
        const juicer = game.mercDeck.all(MercCard).find(m => m.combatantId === 'juicer');
        if (!juicer) {
    ...
    describe('Ewok - +1 action', () => {
      it('Ewok should have 3 actions when reset', () => {
        const ewok = game.mercDeck.all(MercCard).find(m => m.combatantId === 'ewok');
        if (!ewok) {
    ...
    describe('Faustina - +1 training action', () => {
      it('Faustina should have 2 regular actions + 1 training action when reset', () => {
        const faustina = game.mercDeck.all(MercCard).find(m => m.combatantId === 'faustina');
        if (!faustina) {
    ...
    describe('Preaction - Auto-heal', () => {
      it('Preaction should auto-heal at start of day', () => {
        const preaction = game.mercDeck.all(MercCard).find(m => m.combatantId === 'preaction');
        if (!preaction) {
  ...
  describe('Haarg - Squad-based skill bonuses', () => {
    ...
    it('Haarg should get +1 to stats where squadmate has higher base', () => {
      const haarg = game.mercDeck.all(MercCard).find(m => m.combatantId === 'haarg');
      const sonia = game.mercDeck.all(MercCard).find(m => m.combatantId === 'sonia');
  ...
  describe('Combat Ability - Combatant Stats', () => {
    ...
    it('should create combatant with correct hit threshold for Lucid', () => {
      const lucid = game.mercDeck.all(MercCard).find(m => m.combatantId === 'lucid');
      if (!lucid) {
    ...
    it('should handle Ra +1 target bonus', () => {
      const ra = game.mercDeck.all(MercCard).find(m => m.combatantId === 'ra');
      if (!ra) {
  ...
  describe('Equipment-Based Combat Bonuses', () => {
    ...
    it('Bouba should get +1 combat with handgun', () => {
      const bouba = game.mercDeck.all(MercCard).find(m => m.combatantId === 'bouba');
      if (!bouba) {
    ...
    it('Mayhem should get +2 combat with Uzi', () => {
      const mayhem = game.mercDeck.all(MercCard).find(m => m.combatantId === 'mayhem');
      if (!mayhem) {
    ...
    it('Rozeske should get +1 combat with armor', () => {
      const rozeske = game.mercDeck.all(MercCard).find(m => m.combatantId === 'rozeske');
      if (!rozeske) {
  ...
  describe('Vrbansk - Free accessory on hire', () => {
    ...
    it('Vrbansk ability should be defined correctly', () => {
      const vrbansk = game.mercDeck.all(MercCard).find(m => m.combatantId === 'vrbansk');
      if (!vrbansk) {
  ...
  describe('Meatbop - Requires accessory to fight', () => {
    ...
    it('Meatbop should have restriction flag', () => {
      const meatbop = game.mercDeck.all(MercCard).find(m => m.combatantId === 'meatbop');
      if (!meatbop) {
  ...
  describe('Initiative Abilities', () => {
    ...
    it('Kastern should have alwaysFirst flag', () => {
      const kastern = game.mercDeck.all(MercCard).find(m => m.combatantId === 'kastern');
      if (!kastern) {
    ...
    it('Badger should have alwaysBeforeMilitia flag', () => {
      const badger = game.mercDeck.all(MercCard).find(m => m.combatantId === 'badger');
      if (!badger) {
    ...
    it('Khenn rolls D6 for initiative', () => {
      const khenn = game.mercDeck.all(MercCard).find(m => m.combatantId === 'khenn');
      if (!khenn) {
    ...
    it('Vulture ignores initiative penalties from equipment', () => {
      const vulture = game.mercDeck.all(MercCard).find(m => m.combatantId === 'vulture');
      if (!vulture) {
    ...
    it('Walter gives militia +2 initiative', () => {
      const walter = game.mercDeck.all(MercCard).find(m => m.combatantId === 'walter');
      if (!walter) {
  ...
  describe('Combat Action Abilities', () => {
    ...
    it('Basic can reroll once per combat', () => {
      const basic = game.mercDeck.all(MercCard).find(m => m.combatantId === 'basic');
      if (!basic) {
    ...
    it('Golem has preemptive strike', () => {
      const golem = game.mercDeck.all(MercCard).find(m => m.combatantId === 'golem');
      if (!golem) {
    ...
    it('Vandal fires second shot', () => {
      const vandal = game.mercDeck.all(MercCard).find(m => m.combatantId === 'vandal');
      if (!vandal) {
    ...
    it('Adelheid converts militia instead of killing', () => {
      const adelheid = game.mercDeck.all(MercCard).find(m => m.combatantId === 'adelheid');
      if (!adelheid) {
    ...
    it('Wolverine can retarget sixes', () => {
      const wolverine = game.mercDeck.all(MercCard).find(m => m.combatantId === 'wolverine');
      if (!wolverine) {
    ...
    it('Rizen hits new militia target with each hit', () => {
      const rizen = game.mercDeck.all(MercCard).find(m => m.combatantId === 'rizen');
      if (!rizen) {
  ...
  describe('Targeting Abilities', () => {
    ...
    it('Buzzkill prioritizes enemy MERCs', () => {
      const buzzkill = game.mercDeck.all(MercCard).find(m => m.combatantId === 'buzzkill');
      if (!buzzkill) {
    ...
    it('Runde is targeted last', () => {
      const runde = game.mercDeck.all(MercCard).find(m => m.combatantId === 'runde');
      if (!runde) {
  ...
  describe('Dog-Related Abilities', () => {
    ...
    it('Shadkaam is immune to attack dogs', () => {
      const shadkaam = game.mercDeck.all(MercCard).find(m => m.combatantId === 'shadkaam');
      if (!shadkaam) {
    ...
    it('Tao will not harm dogs', () => {
      const tao = game.mercDeck.all(MercCard).find(m => m.combatantId === 'tao');
      if (!tao) {
  ...
  describe('Surgeon - Sacrifice die to heal', () => {
    ...
    it('Surgeon ability should be defined', () => {
      const surgeon = game.mercDeck.all(MercCard).find(m => m.combatantId === 'surgeon');
      if (!surgeon) {
  ...
  describe('Squad Bonus Abilities', () => {
    ...
    it('Valkyrie gives +1 initiative to squadmates', () => {
      const valkyrie = game.mercDeck.all(MercCard).find(m => m.combatantId === 'valkyrie');
      if (!valkyrie) {
    ...
    it('Tack gives +2 initiative to squad when she has highest', () => {
      const tack = game.mercDeck.all(MercCard).find(m => m.combatantId === 'tack');
      if (!tack) {
    ...
    it('Snake gets +1 to all stats when alone', () => {
      const snake = game.mercDeck.all(MercCard).find(m => m.combatantId === 'snake');
      if (!snake) {
    ...
    it('Sarge gets +1 to all stats when highest initiative', () => {
      const sarge = game.mercDeck.all(MercCard).find(m => m.combatantId === 'sarge');
      if (!sarge) {
    ...
    it('Tavisto gets +1 to all stats with woman in squad', () => {
      const tavisto = game.mercDeck.all(MercCard).find(m => m.combatantId === 'tavisto');
      if (!tavisto) {
    ...
    it('Max gives -1 combat to enemy MERCs', () => {
      const max = game.mercDeck.all(MercCard).find(m => m.combatantId === 'max');
      if (!max) {
  ...
  describe('Special Equipment Abilities', () => {
    ...
    it('Moe gets +1 target with SMAW', () => {
      const moe = game.mercDeck.all(MercCard).find(m => m.combatantId === 'moe');
      if (!moe) {
    ...
    it('Dutch gets +1 combat and +1 initiative unarmed', () => {
      const dutch = game.mercDeck.all(MercCard).find(m => m.combatantId === 'dutch');
      if (!dutch) {
    ...
    it('Vandradi gets +1 combat with multi-target weapon', () => {
      const vandradi = game.mercDeck.all(MercCard).find(m => m.combatantId === 'vandradi');
      if (!vandradi) {
  ...
  describe('Squidhead - Land mine handling', () => {
    ...
    it('Squidhead ability should be defined', () => {
      const squidhead = game.mercDeck.all(MercCard).find(m => m.combatantId === 'squidhead');
      if (!squidhead) {
