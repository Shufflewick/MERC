...
var init_game_element = __esm({
  "../../../../board_game_service/BoardSmith/packages/engine/dist/element/game-element.js"() {
    ...
    GameElement = class _GameElement {
       ...
       */
      resolveElementReferences(game) {
        const unserializable = new Set(this.constructor.unserializableAttributes);
        for (const key of Object.keys(this)) {
          if (!unserializable.has(key) && !key.startsWith("_")) {
            const value = this[key];
            if (value !== void 0) {
              this[key] = this.deserializeValue(value, game);
            }
          }
        }
        for (const child of this._t.children) {
...
function createMoveAction(game) {
  return Action.create("move").prompt("Move your squad").condition((ctx) => {
    return game.isRebelPlayer(ctx.player);
  }).chooseElement("squad", {
...
function createHireStartingMercsAction(game) {
  const drawnMercsCache = /* @__PURE__ */ new Map();
  return Action.create("hireStartingMercs").prompt("Hire your starting MERCs").notUndoable().condition((ctx) => {
    if (!game.isRebelPlayer(ctx.player)) return false;
    const player = ctx.player;
...
function createEquipStartingAction(game) {
  return Action.create("equipStarting").prompt("Equip starting equipment").notUndoable().condition((ctx) => {
    if (!game.isRebelPlayer(ctx.player)) return false;
    const player = ctx.player;
...
function createPlaceLandingAction(game) {
  return Action.create("placeLanding").prompt("Choose your landing zone").condition((ctx) => {
    return game.isRebelPlayer(ctx.player);
...
function createDictatorPlaceInitialMilitiaAction(game) {
  return Action.create("dictatorPlaceInitialMilitia").prompt("Place initial militia on unoccupied industries").condition(() => {
...
function createDictatorHireFirstMercAction(game) {
  return Action.create("dictatorHireFirstMerc").prompt("Hire your first MERC").condition(() => {
    return game.currentDay === 1;
  }).execute(() => {
    hireDictatorMerc(game);
...
function createDictatorSetupAbilityAction(game) {
  return Action.create("dictatorSetupAbility").prompt("Apply dictator special ability").condition(() => {
    return game.currentDay === 1;
  }).execute(() => {
    applyDictatorSetupAbility(game);
...
function createDictatorDrawTacticsAction(game) {
  return Action.create("dictatorDrawTactics").prompt("Draw tactics cards").condition(() => {
    return game.currentDay === 1;
  }).execute(() => {
    drawTacticsHand(game);
...
function createDictatorPlaceExtraMilitiaAction(game) {
  return Action.create("dictatorPlaceExtraMilitia").prompt("Place extra militia").condition(() => {
...
function createDictatorSkipExtraMilitiaAction(game) {
  return Action.create("dictatorSkipExtraMilitia").prompt("No extra militia to place").condition(() => {
    return game.currentDay === 1 && game.setupConfig.dictatorStrength.extra === 0;
  }).execute(() => {
...
function hexToPlayerColor(hex) {
  const colorMap = {
    ...
    "#f1c40f": "yellow",
    "#95a5a6": "black",
    // Gray/dictator color maps to black
    "#ecf0f1": "black"
    // White also maps to black for now
  };
...
var MERCGame = class _MERCGame extends Game {
  ...
  static _pendingDictatorPosition = -1;
  // Player configurations from lobby (colors, AI settings)
  static _pendingPlayerConfigs = [];
  // Configuration
  rebelCount;
  setupConfig;
  currentDay = 1;
  ...
  get rebelPlayers() {
    return this.players.filter((p) => p instanceof RebelPlayer);
  }
  ...
  constructor(options) {
    ...
    _MERCGame._pendingPlayerConfigs = options.playerConfigs || [];
    let dictatorPos = options.dictatorPlayerPosition ?? -1;
    const configDictatorIndex = _MERCGame._pendingPlayerConfigs.findIndex((c) => c.role === true);
    if (configDictatorIndex >= 0) {
      dictatorPos = configDictatorIndex;
    }
    if (dictatorPos >= 0 && dictatorPos >= _MERCGame._pendingPlayerCount) {
      throw new Error(
        `Invalid dictator position: ${dictatorPos}. Must be less than playerCount (${_MERCGame._pendingPlayerCount}).`
      );
    }
    _MERCGame._pendingDictatorPosition = dictatorPos;
    super(options);
    ...
    if (!this.dictatorPlayer) {
      const dictPos = _MERCGame._pendingDictatorPosition >= 0 ? _MERCGame._pendingDictatorPosition : _MERCGame._pendingPlayerCount - 1;
      throw new Error(
        `DictatorPlayer not created. Players: ${this.players.length}, pendingCount: ${_MERCGame._pendingPlayerCount}, dictatorPosition: ${dictPos}`
      );
    }
    if (options.dictatorIsAI !== void 0) {
  ...
  createPlayer(position, name) {
    ...
    const dictatorPosition = _MERCGame._pendingDictatorPosition >= 0 ? _MERCGame._pendingDictatorPosition : totalPlayers - 1;
    ...
    if (isDictator) {
      ...
      rebel.game = this;
      const playerConfig = _MERCGame._pendingPlayerConfigs[position];
...
// ../../../../board_game_service/BoardSmith/packages/session/dist/colors.js
var STANDARD_PLAYER_COLORS = [
...
function createColorOption(colors, label = "Color") {
  return {
    type: "color",
    label,
...
// ../../../../board_game_service/BoardSmith/packages/runtime/dist/index.js
init_dist();
init_dist();
init_dist();

// ../../../../board_game_service/BoardSmith/packages/runtime/dist/runner.js
init_dist();

// ../../../../board_game_service/BoardSmith/packages/ai/dist/mcts-bot.js
init_dist();
  ...
  presets: [
    {
      name: "Play as Rebel",
      players: [
        { color: "#e74c3c", role: false, isAI: false },
        // Red rebel (you)
        { color: "#95a5a6", role: true, isAI: true, aiLevel: "medium" }
        // Dictator AI
      ]
    },
    {
      ...
      players: [
        { color: "#95a5a6", role: true, isAI: false },
        // Dictator (you)
        { color: "#e74c3c", role: false, isAI: true, aiLevel: "medium" }
        // Red rebel AI
      ]
    },
    {
      ...
      players: [
        { color: "#e74c3c", role: false, isAI: false },
        // Red rebel
        { color: "#3498db", role: false, isAI: false },
        // Blue rebel
        { color: "#95a5a6", role: true, isAI: true, aiLevel: "medium" }
        // Dictator AI
      ]
    },
    {
      ...
      players: [
        ...
        { color: "#3498db", role: false, isAI: false },
        // Blue rebel
        { color: "#27ae60", role: false, isAI: false },
        // Green rebel
        { color: "#95a5a6", role: true, isAI: true, aiLevel: "hard" }
        // Dictator AI
      ]
    }
  ]
